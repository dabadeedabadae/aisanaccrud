...
(function() {
	...
	function initializeChat() {
				...
				if (xhr.status === 200) {
					try {
						...
						if (config.welcomeMessage) {
							...
							addMessage('bot', 'Здравствуйте! Я виртуальный помощник приемной комиссии СКУ им. М. Козыбаева. Чем могу помочь?');
						}
						...
						console.error('Error parsing config:', e, xhr.responseText);
						messagesEl.innerHTML = '';
						addMessage('bot', 'Здравствуйте! Я виртуальный помощник приемной комиссии СКУ им. М. Козыбаева. Чем могу помочь?');
						isInitialized = true;
					}
				} else {
					console.error('HTTP Error:', xhr.status, configUrl);
					messagesEl.innerHTML = '';
					addMessage('bot', 'Здравствуйте! Я виртуальный помощник приемной комиссии СКУ им. М. Козыбаева. Чем могу помочь?');
					isInitialized = true;
				}
		...
		xhr.onerror = function() {
			console.error('Network error:', configUrl);
			messagesEl.innerHTML = '';
			addMessage('bot', 'Здравствуйте! Я виртуальный помощник приемной комиссии СКУ им. М. Козыбаева. Чем могу помочь?');
			isInitialized = true;
		};
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState === 4) {
				...
				if (xhr.status === 200) {
					...
					console.error('HTTP Error:', xhr.status, apiUrl);
					addMessage('bot', 'Ошибка соединения с сервером. Пожалуйста, попробуйте позже или обратитесь в приемную комиссию по телефону: +7 (7172) 55-55-55');
				}
			}
		};
		xhr.onerror = function() {
			console.error('Network error:', apiUrl);
			if (loadingMsg && loadingMsg.parentNode) {
				loadingMsg.parentNode.removeChild(loadingMsg);
			}
			addMessage('bot', 'Ошибка соединения с сервером. Пожалуйста, попробуйте позже или обратитесь в приемную комиссию по телефону: +7 (7172) 55-55-55');
